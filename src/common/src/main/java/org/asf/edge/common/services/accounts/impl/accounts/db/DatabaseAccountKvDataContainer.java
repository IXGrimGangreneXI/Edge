...
public class DatabaseAccountKvDataContainer extends AccountKvDataContainer {
	...
	public DatabaseAccountKvDataContainer(String table, AccountObject account, String id,
		...
		try {
			...
			try {
				...
				var statement = req.prepareStatement("SELECT DATA FROM " + table
						+ " WHERE DATAKEY = ? AND PARENT = ? AND PARENTCONTAINER = ? AND ACCID = ?");
		...
		try {
			...
			try {
				...
				var statement = req.prepareStatement("UPDATE " + table
						+ " SET DATA = ? WHERE DATAKEY = ? AND PARENT = ? AND PARENTCONTAINER = ? AND ACCID = ?");
		...
		try {
			...
			try {
				...
				var statement = req.prepareStatement(
						"SELECT DATAKEY FROM " + table + " WHERE PARENT = ? AND PARENTCONTAINER = ? AND ACCID = ?");
	...
	protected String[] getChildContainers(String key) throws IOException {
		...
		try {
			...
			try {
				// Create prepared statement
				var statement = req
						.prepareStatement("SELECT PARENT FROM " + table + " WHERE PARENTCONTAINER = ? AND ACCID = ?");
				statement.setString(1, key);
	...
	protected void deleteContainer(String root) throws IOException {
		...
		try {
			...
			try {
				// Create prepared statement
				var statement = req.prepareStatement(
						"DELETE FROM " + table + " WHERE PARENT = ? AND PARENTCONTAINER = ? AND ACCID = ?");
				statement.setString(1, parent);
				statement.setString(2, parentContainer);
				statement.setString(3, id);
				statement.execute();
	...
	protected JsonElement find(BiFunction<String, JsonElement, Boolean> function, String root) throws IOException {
		...
		try {
			...
			try {
				// Create prepared statement
				var statement = req.prepareStatement(
						"SELECT * FROM " + table + " WHERE PARENT = ? AND PARENTCONTAINER = ? AND ACCID = ?");
				statement.setString(1, parent);
				statement.setString(2, parentContainer);
				statement.setString(3, id);
				ResultSet res = statement.executeQuery();
