...
public class DatabaseSaveKvDataContainer extends AccountKvDataContainer {
	...
	public DatabaseSaveKvDataContainer(String table, AccountObject account, AccountSaveContainer save,
	...
	protected JsonElement get(String key) throws IOException {
		try {
			...
			try {
				...
				var statement = req.prepareStatement("SELECT DATA FROM " + table
						+ " WHERE DATAKEY = ? AND PARENT = ? AND PARENTCONTAINER = ? AND SVID = ?");
	...
	protected void set(String key, JsonElement value) throws IOException {
		try {
			...
			try {
				...
				// Create prepared statement
				var statement = req.prepareStatement("UPDATE " + table
						+ " SET DATA = ? WHERE DATAKEY = ? AND PARENT = ? AND PARENTCONTAINER = ? AND SVID = ?");
	...
	protected void create(String key, JsonElement value) throws IOException {
		try {
			...
			try {
				...
				// Create prepared statement
				var statement = req.prepareStatement("INSERT INTO " + table + " VALUES(?, ?, ?, ?, ?)");
	...
	protected void delete(String key) throws IOException {
		try {
			...
			try {
				...
				// Create prepared statement
				var statement = req.prepareStatement("DELETE FROM " + table
						+ " WHERE DATAKEY = ? AND PARENT = ? AND PARENTCONTAINER = ? AND SVID = ?");
				statement.setString(1, key);
				statement.setString(2, parent);
				statement.setString(3, parentContainer);
		...
		try {
			...
			try {
				...
				var statement = req.prepareStatement(
						"SELECT DATAKEY FROM " + table + " WHERE PARENT = ? AND PARENTCONTAINER = ? AND SVID = ?");
	...
	protected String[] getChildContainers(String key) throws IOException {
		...
		try {
			...
			try {
				// Create prepared statement
				var statement = req
						.prepareStatement("SELECT PARENT FROM " + table + " WHERE PARENTCONTAINER = ? AND SVID = ?");
				statement.setString(1, key);
	...
	protected void deleteContainer(String root) throws IOException {
		...
		try {
			...
			try {
				// Create prepared statement
				var statement = req.prepareStatement(
						"DELETE FROM " + table + " WHERE PARENT = ? AND PARENTCONTAINER = ? AND SVID = ?");
				statement.setString(1, parent);
				statement.setString(2, parentContainer);
				statement.setString(3, id);
				statement.execute();
	...
	protected JsonElement find(BiFunction<String, JsonElement, Boolean> function, String root) throws IOException {
		...
		try {
			...
			try {
				// Create prepared statement
				var statement = req.prepareStatement(
						"SELECT * FROM " + table + " WHERE PARENT = ? AND PARENTCONTAINER = ? AND SVID = ?");
				statement.setString(1, parent);
				statement.setString(2, parentContainer);
				statement.setString(3, id);
				ResultSet res = statement.executeQuery();
