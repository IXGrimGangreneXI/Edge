...
/**
 * 
 * Grid API server setup event - called before the server starts
 * 
 * @author Sky Swimmer
 *
 */
@EventPath("gridapi.server.setup")
public class GridApiServerSetupEvent extends GridApiServerEvent {
	private GridApiServerConfig config;
	private EdgeGridApiServer server;

	public GridApiServerSetupEvent(GridApiServerConfig config, EdgeGridApiServer server) {
		this.config = config;
		this.server = server;
	}
